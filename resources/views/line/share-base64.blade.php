         ...
         window.vueConfig = {
           ...
           async mounted () {
             try {
               ...
               await this.swalFire({ icon: 'error', title: '程式錯誤', text: err.message })
             }
             this.loading = false
           ...
           methods: {
             async getTpl () {
               try {
                 ...
                 if (!this.paramBase64url('shared')) throw new Error('shared is required.')
                 if (!window.getSearchParam('type')) throw new Error('type is required.')
                 if(window.getSearchParam('type')=='message'){
                    this.msgs=JSON.parse(this.paramBase64url('shared'))
                 }
